 ...
 */
@AggregationFunction("bitmap_union")
public final class BitmapUnionAggregation {
    public BitmapUnionAggregation() {
    }

    @InputFunction
    public static void bitmapOr(@AggregationState SliceState state, @SqlType(StandardTypes.VARBINARY) Slice value) {
        try {
            if (state.getSlice() == null) {
                state.setSlice(value);
            } else {
                RoaringBitmap rb = BitmapOperators.or(state.getSlice(), value);
    ...
    public static void combine(@AggregationState SliceState state, @AggregationState SliceState otherState) {
        try {
            if (state.getSlice() == null) {
                ...
                RoaringBitmap rb = BitmapOperators.or(state.getSlice(), otherState.getSlice());
                state.setSlice(Slices.wrappedBuffer(serialize(rb)));
            }
        } catch (IOException e) {
            e.printStackTrace();
